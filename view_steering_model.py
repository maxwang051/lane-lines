...
output_directory = '../lane-lines/path_images/'
...
'''
  ...
  for i in range(skip*100, log['times'].shape[0]):
    ...
    cpy_img = img
    draw_path_on(cpy_img, speed_ms, -angle_steers/10.0)
    #draw_path_on(cpy_img, int(speed_ms), int(-predicted_steers/10.0), (0, 255, 0))
    # draw on
    pygame.surfarray.blit_array(camera_surface, img.swapaxes(0,1))
    camera_surface_2x = pygame.transform.scale2x(camera_surface)
    screen.blit(camera_surface_2x, (0,0))
    pygame.display.flip()
    cv2.imwrite(output_directory + str(i) + '.jpg', cpy_img)
